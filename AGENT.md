...
- Advisor client books sync through Supabase `public.advisor_clients` (parsed analysis JSON + notes per user). Apply `supabase/migrations/20260610000000_advisor_clients.sql` before deploying frontend builds that call the table.
- Advisor Review Platform tables: `review_snapshots`, `review_links`, `meeting_briefs`, `client_review_events`. Apply `supabase/migrations/20260611000000_advisor_review_platform.sql`.
- Review APIs: `POST /api/reviews/prepare`, `POST /api/reviews/share`, `POST /api/reviews/snapshot`, `GET /api/reviews/public/{share_id}` (public), `PATCH /api/reviews/links/{id}`, `GET /api/reviews/history/{client_pan}`, `GET /api/reviews/compare`.
- Public client review page: `/review/:shareId` (no auth). LLM via `LLM_PROVIDER` (`gemini` default, or `openai` / `anthropic`). Set `GEMINI_API_KEY` (and optional `GEMINI_MODEL`, default `gemini-2.0-flash`), or `OPENAI_API_KEY` / `ANTHROPIC_API_KEY`. Falls back to template summaries when no provider key is set.
