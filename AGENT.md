...
- CAS PDF parsing runs in a killable subprocess when available, with `PDF_PARSE_TIMEOUT_SECONDS` capped between 1 and 120 seconds and defaulting to 60. `PDF_PARSE_EXECUTOR=auto` falls back to thread-based parsing on hosts that block child process startup.
