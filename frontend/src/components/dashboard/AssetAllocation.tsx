...
const ALLOC_COLORS = ["#10b981", "#818cf8", "#cbd5e1"]
...
function AssetAllocationInner({ summary }: AssetAllocationProps) {
  const { tableData, pieData, equityPct } = useMemo(() => {
    ...
    const pieData = [
      { name: "Equity", value: equity, color: ALLOC_COLORS[0] },
      { name: "Fixed Income", value: debt, color: ALLOC_COLORS[1] },
      { name: "Others", value: others, color: ALLOC_COLORS[2] },
    ].filter((d) => d.value > 0)
    return {
      tableData: alloc,
      pieData,
    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6 lg:gap-8 mb-8 sm:mb-12">
      <Card className="border-slate-100">
        <CardContent className="p-4 sm:p-6 lg:p-8 flex flex-col items-center justify-center relative">
          ...
          <h3 className="w-full text-left font-bold text-base sm:text-lg text-slate-800 mb-4 sm:mb-6">
            Asset Allocation
          </h3>
